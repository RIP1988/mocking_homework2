...
public class WriteFileMockTest {
	...
	// Mockito.doNothing().when(writeFile).areaInFile(Mockito.any(JTextArea.class),
	// Mockito.any(String.class));
	// //czy powyzsza konstrukcja jest poprawna, zeby uniknac wywolania
	// rzeczywistej implementacji areaInFile?
	// ActionEvent actionEventMock = Mockito.mock(ActionEvent.class);
	// Mockito.when(textFieldMock.getText()).thenReturn("to-mocking-instrukcja.docx");
	// //czy tak moze byc, zeby getText() w if zwracalo true?
	// writeFile.actionPerformed(actionEventMock);
	// verify(pathPanel, times(1)).sayFileSaved();//co mam zrobic z ta
	// niedzialajaca metoda?
	// //czy moze zamiast pathPanel powinno byc write file, a zamiast
	// .sayFileSaved() .PathPanel.sayFileSaved()?
	// //no i jak w przypadku Read, dlaczego to times nie dziala?
	// //czy ogolnie jakiejs deklaracji jeszcze brakuje w calej metodzie
	// testowej?
	// }
